...
// Campos de Artist que referencian catálogos paramétricos (Country/Language).
// El front puede reenviar el id que la propia API le mostró para esos catálogos
// (GET /countries, GET /languages), que ahora es su sID enmascarado en vez del
// ObjectId real -- hay que resolverlo antes de guardar.
  ...
  artistRouter.get(
    ...
    async (req, res) => {
      ...
      try {
        ...
        query.$or = [{ sID: artistId }, { username: artistId }, { name: artistId }];
  ...
  artistRouter.post(
    ...
    async (req, res) => {
      try {
        ...
        await resolveReferenceFields(info, { environment: req.serverEnvironment });
  ...
  artistRouter.put(
    ...
    async (req, res) => {
      ...
      try {
        ...
        await resolveReferenceFields(newInfo, { environment: req.serverEnvironment });
        ...
        if (!artist) {
          ...
          if (hasRole) {
            ...
            return res
              ...
              .send(
                createPaginatedDataResponse(
                  maskIds(updatedArtist.toObject()),
                ),
              );
          } else {
