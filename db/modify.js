...
async function main() {
  ...
  */
  ...
  await connectToDatabase();
    ...
    await EntityDirectory.deleteMany({
      name: "Pachamoro",
    });
    */
  // exit()
...
async function registrarEventos() {
  ...
  let books = JSON.parse(
    fs.readFileSync("./assets/mocks/domain/events/eventosRealesCompletos.json")
  );
...
async function extractRealEvents() {
  ...
  try {
    ...
    let books = JSON.parse(
      fs.readFileSync("./assets/mocks/domain/events/eventosReales.json")
    );
    ...
    const placesUsernames = [
      ...new Set(books.map((element) => element["place"])),
    ];
    ...
    let placesWithIds = await Promise.all(
      placesUsernames.map(async (username) => {
        const place = await Place.findOne({ username })
          .select(["_id", "name"])
          .exec();
        return {
          username,
    ...
    placesWithIds = placesWithIds.filter(
      (place) => !!place.username && !!place.id
    );

    let artistsWithIds = await Promise.all(
      artistsUsernames.map(async (username) => {
        const place = await Artist.findOne({ username })
          .select(["_id", "name"])
          .exec();
        return {
          username,
    ...
    artistsWithIds = artistsWithIds.filter(
      (place) => !!place.username && !!place.id
    );

    console.log(artistsWithIds);
    books.forEach(async (event) => {
      const place = placesWithIds.find(
        (place) => place.username === event["place"]
      );
      const allArtists = event.artists.every((artist) =>
        artistsWithIds.find((artistId) => artistId.username === artist)
      );
      ...
      if (place && allArtists) {
        ...
        event.artists = event.artists.map(
          (a) =>
            new mongoose.Types.ObjectId(
              artistsWithIds.find((aId) => a === aId.username).id
            )
        );
    ...
    fs.writeFileSync(
      "./assets/mocks/domain/events/eventosRealesCompletos.json",
      JSON.stringify(completos, null, 2),
      "utf-8"
    );
    fs.writeFileSync(
      "./assets/mocks/domain/events/eventosRealesFaltantes.json",
...
function generateEventTemplate(data) {
  ...
  return {
    ...
    num: data["id"] || "RRR",
    fullprice: Number(
      data["Cover Full"]?.replace(".", "").replace("$", "") || 0
    ),
  };
}
